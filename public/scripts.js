function renderApartments(parameters) {
    ...
    <ul>
      ${parameters
        .map((apart) => {
            return `
            <a href="/apartment/${apart.apartment_id}" class="productClicked" data-id="${apart.apartment_id}">
              <li class="card">
                <strong>${apart.title}</strong>
                <strong class="price">Ціна: ${apart.price} грн</strong>
                <p>Опис: ${apart.description?.full_text || "Опис відсутній"}</p>
                <ul>
                  <li><strong>Кімнат:</strong> ${apart.params?.rooms ?? "-"}</li>
                  <li><strong>Площа (м²):</strong> ${apart.params?.area ?? "-"}</li>
                  <li><strong>Поверх:</strong> ${apart.params?.floor ?? "-"}</li>
                  <li><strong>Адреса:</strong> ${apart.params?.address ?? "-"}</li>
                </ul>
              </li>
            </a>
          `;
...
async function updateUI(page = 1) {
    ...
    const count = sortRooms ? sortRooms.value : "";
...
if (applyFilters) {
    applyFilters.addEventListener("click", () => updateUI(1));
}
